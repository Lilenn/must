...
# TemplateResponseMixin 这个mixin提供的功能是渲染模板并且返回http响应，需要一个template_name属性用于指定模板位置，
# 还提供render_to_response 方法给模板传入上下文并且渲染模板
...
class ContentDeleteView(View):
    def post(self, request, id):
        ...
        # 删除content对象
        content.delete()
        # 之后重定向到了module_content_list url
...
class CourseListView(TemplateResponseMixin, View):
    ...
    def get(self, request, subject=None):
        # 取得所有的主题，使用annotate分组，count聚合方法生一个包含课程数量的字段
        ...
        if subject:
            ...
            # 缓存只能是用于存储最终可供使用的查询结果，不能再中间步骤缓存
...
class CourseDetailView(DetailView):
    ...
    # 这里重写get_。。。data方法，把这个表单添加到模板变量中，并且将表单隐藏的字段内容初始化成当前的课程对象
