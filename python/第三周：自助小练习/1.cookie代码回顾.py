...
# # build_opener 内部的实现就是 urlopen
# # urlopen 只能进行简单的请求 不支持在线验证cookie 和代理ip等复杂操作
...
# }
# request = Request('http://www.meishij.net/')
# response =opener.open(request)
# print(response.read().decode())
# -------------------------------------END--------------------------------
from http.cookiejar import CookieJar,LWPCookieJar
...
# 生成一个管理cookie的对象
cookie_obj = CookieJar()
# 常见一个支持cookie的对象 对象属于HTTPCookieProcesssor
cookie_handler = HTTPCookieProcessor(cookie_obj)
# build_opener 内部的实现就是urlopen
# urlopen 只能进行简单的请求 不支持在线验证 cookie 和代理ip
opener = build_opener(cookie_handler)
response = opener.open('http://www.neihanshequ.com')
for cookie in cookie_obj:
    print('key',cookie.name)
...
cookie_handler = HTTPCookieProcessor(cookie_obj)
opener = build_opener(cookie_handler)
response = opener.open('http://www.neishanshequ.com')
# 保存cookie到指定的文件中去
# ignore 忽略
# ignore_expires=True 即便目标cookie已经存在于文件当中，仍然将其写入
# ignore_discard=True 即便目标cookie将要/已经过期，仍然将其写入
cookie_obj.save(ignore_expires=True,ignore_discard=True)
...
post_url = 'https://i.meishi.cc/login.php?redirect=https%3A%2F%2Fi.meishi.cc%2Flogin.php%3Fac%3Dzhuce'
# urlencode 是对url当中的参数进行编码 同时编码的对象为 字典
# quote 是对url中的中文进行编码 编码的对象为 字符串
post_data = urlencode({
    'username':'846077763@qq.com',
    'password':'doudou12345.'
})
